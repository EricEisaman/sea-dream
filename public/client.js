...
document.querySelector('a-scene').addEventListener('loaded', function () {
  ...
  window.addOtherPlayer = newPlayerObject=>{
    ...
    p.msg.setAttribute('position',window.config.msg.offset);
 ...
 window.otherPlayers[id].parentNode.removeChild(window.otherPlayers[id]);
  ...
  function update(dt){
    totalTime += dt;
    totalSteps++;
    let py = player.object3D.position.y
    if(py < (window.config.avatar.height - 1)){
        let x = player.object3D.position.x;
        let z = player.object3D.position.z;
        player.body.el.setAttribute("position",`${x} ${window.config.avatar.height} ${z}`);
        player.body.el.setAttribute("velocity","0 0 0");
      }
    if(py >50){
        let x = player.object3D.position.x;
        let z = player.object3D.position.z;
        player.body.el.setAttribute("position",`${x} 50 ${z}`);
        player.body.el.setAttribute("velocity","0 0 0");
      }
    let vy = player.getAttribute('velocity').y;
    if(Math.abs(vy) > 3 && py >4){
        let vx = player.getAttribute('velocity').x;
        let vz = player.getAttribute('velocity').z;
        player.body.el.setAttribute("velocity",`${vx} ${vy*0.6} ${vz}`);
      }
    if(totalSteps%6 == 0) {
      let playerData = {};
      let pos = player.getAttribute('position');
      ...
      playerData.thrust = player.isThrusting && window.config.thruster.activated;
